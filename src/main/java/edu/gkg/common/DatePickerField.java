...
/**
 * 可弹出日历的日期选择控件（纯 Swing，无需第三方库）。
 *
 * 使用方式：
 *   DatePickerField picker = new DatePickerField(LocalDate.now().minusDays(1));
 *   picker.setOnChange(date -> System.out.println(date));
 *   LocalDate selected = picker.getSelectedDate();
 *
 * 点击文本框或 ▼ 按钮弹出月历，点击日期格子即选中并关闭弹窗。
 * 可通过 setMinDate / setMaxDate 禁用超范围日期（灰显且不可点）。
 */
public class DatePickerField extends JPanel {
    ...
    private LocalDate minDate = LocalDate.of(2015, 2, 19); // GDELT v2 起始
    ...
    private JPanel buildGrid() {
        ...
        int startDow = first.getDayOfWeek().getValue() % 7; // 0=日 1=一 … 6=六
        for (int i = 0; i < startDow; i++) grid.add(new JLabel()); // 空格占位

        int days = viewMonth.lengthOfMonth();
        for (int d = 1; d <= days; d++) {
            LocalDate day = viewMonth.atDay(d);
            grid.add(buildDayCell(day));
        }
        return grid;
    }
    ...
    private JLabel buildDayCell(LocalDate day) {
        ...
        if (isSelected) {
            ...
            cell.setForeground(Theme.TEXT_MUTED);
        } else if (isToday) {
            cell.setForeground(Theme.BRAND);
        } else {
            cell.setForeground(Theme.TEXT_PRIMARY);
        }
        ...
        if (!disabled) {
            ...
            cell.addMouseListener(new MouseAdapter() {
                @Override public void mouseEntered(MouseEvent e) {
                    if (!isSelected) cell.setBackground(Theme.BG_HOVER);
